...
PROBLEM_P = b"DDCCBABA      "  # xdavidliu actual input
...
for r0 in range(4):
    for r1 in range(4):
        if r0 >= r1:
            continue  # within room dist not needed; also r0 > r1 handled by symmetry
        for k0 in range(H):
            for k1 in range(H):
    ...
    while improved:
        ...
        if not all(is_partial):
            for r in range(4):
                ...
                c = p[ir]
                rd = c - ord("A")
                if not (is_partial[rd] and ClearBetweenRoom(p, r, rd)):
                    continue
...
def Dijkstra(p0: bytes) -> int:
    ...
    while hp:
        ...
        for r in range(4):
            ...
            for h in range(7):
                ih = H4 + h
                if not (p[ih] == SPACE and ClearHallwayToRoom(p, h, r)):
                    continue
                pa = bytearray(p)
