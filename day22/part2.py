...
# parse input
data = open("22.in", "r", encoding="utf-8").read().splitlines()
...
xs = sorted(
    {cube.x1 - 1 for _, cube in steps}
    | {cube.x1 for _, cube in steps}
    | {cube.x1 + 1 for _, cube in steps}
    | {cube.x2 - 1 for _, cube in steps}
    | {cube.x2 for _, cube in steps}
    | {cube.x2 + 1 for _, cube in steps}
)
ys = sorted(
    {cube.y1 - 1 for _, cube in steps}
    | {cube.y1 for _, cube in steps}
    | {cube.y1 + 1 for _, cube in steps}
    | {cube.y2 - 1 for _, cube in steps}
    | {cube.y2 for _, cube in steps}
    | {cube.y2 + 1 for _, cube in steps}
)
zs = sorted(
    {cube.z1 - 1 for _, cube in steps}
    | {cube.z1 for _, cube in steps}
    | {cube.z1 + 1 for _, cube in steps}
    | {cube.z2 - 1 for _, cube in steps}
    | {cube.z2 for _, cube in steps}
    | {cube.z2 + 1 for _, cube in steps}
)
